...
export class LineraClient {
  ...
  // Helper method to query player's own chain
  async queryPlayerChain(query) {
    try {
      if (!this.counter) {
        throw new Error("Client not initialized");
      }
      ...
      console.error("Failed to query player chain:", error);
      throw error;
    }
  }
   ...
   * @param {string} sessionId - Unique session ID generated by frontend
   * @param {string} username - The username of the player
   * @param {string|null} tournamentId - Optional tournament ID if playing in a tournament
   * @returns {Promise<Object>} Session data with session_id
   */
  async startGameSession(sessionId, username, tournamentId = null) {
    const mutation = `
      mutation {
        startGameSession(
          sessionId: "${sessionId}"
    ...
    try {
      const response = await this.queryPlayerChain(mutation);
      return response;
    } catch (error) {
  ...
  /**
   * Submit a verified score with game proof
   * @param {string} sessionId - The session ID from startGameSession
   * @param {string} username - The username of the player
   * @param {string|null} tournamentId - Optional tournament ID
   * @param {Object} proof - Game proof object with pipes_passed, game_duration_ms, jump_count, final_score
   * @returns {Promise<Object>} Response from blockchain
   */
  async submitVerifiedScore(sessionId, username, tournamentId, proof) {
    const mutation = `
      mutation {
        submitVerifiedScore(
          sessionId: "${sessionId}"
          username: "${username}"
          ${tournamentId ? `tournamentId: "${tournamentId}"` : "tournamentId: null"}
          proof: {
            pipesPassed: ${proof.pipes_passed}
            gameDurationMs: ${proof.game_duration_ms}
            jumpCount: ${proof.jump_count}
            finalScore: ${proof.final_score}
          }
        )
      }
    `;
  ...
  /**
   * Get proof history for the current player chain
   * @returns {Promise<Array>} Array of proof history entries
   */
  async getProofHistory() {
    ...
    try {
      const response = await this.queryPlayerChain(query);
      console.log("getProofHistory response:", response);

      // Handle different response formats
      if (response && response.proofHistory) {
        return response.proofHistory;
  ...
  /**
   * Get a specific proof entry by session ID
   * @param {string} sessionId - The session ID
   * @returns {Promise<Object|null>} Proof entry or null
   */
  async getProofEntry(sessionId) {
    const query = `
      query {
        proofEntry(sessionId: "${sessionId}") {
          proof {
            pipesPassed
            gameDurationMs
            jumpCount
    ...
    try {
      const response = await this.queryPlayerChain(query);
      return response.proofEntry || null;
    } catch (error) {
      console.error("Failed to get proof entry:", error);
      throw error;
    }
  }
  ...
  /**
   * Get accepted proofs
   * @returns {Promise<Array>} Array of accepted proof entries
   */
  async getAcceptedProofs() {
    const query = `
      query {
        acceptedProofs {
          proof {
    ...
    try {
      const response = await this.queryPlayerChain(query);
      return response.acceptedProofs || [];
    } catch (error) {
      console.error("Failed to get accepted proofs:", error);
      throw error;
    }
  }
  ...
  /**
   * Get rejected proofs
   * @returns {Promise<Array>} Array of rejected proof entries
   */
  async getRejectedProofs() {
    const query = `
      query {
        rejectedProofs {
          proof {
    ...
    try {
      const response = await this.queryPlayerChain(query);
      return response.rejectedProofs || [];
    } catch (error) {
      console.error("Failed to get rejected proofs:", error);
      throw error;
    }
  }
  ...
  /**
   * Get pending proofs
   * @returns {Promise<Array>} Array of pending proof entries
   */
  async getPendingProofs() {
    ...
    try {
      const response = await this.queryPlayerChain(query);
      return response.pendingProofs || [];
    } catch (error) {
      console.error("Failed to get pending proofs:", error);
      throw error;
    }
  }
}
